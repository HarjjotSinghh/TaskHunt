...
const Header = () => {
  ...
  return (
    <>
      <nav className="backdrop-blur-lg z-[999] fixed text-[18px]">
        <div className="w-screen flex flex-wrap items-center justify-between p-4 lg:px-12 px-2 first:mr-auto last:ml-auto">
            ...
            <div className="relative">
              {user ? (
                <button type="button" className="flex lg:pl-16 pl-0 lg:mr-3 items-center justify-center text-sm  rounded-full mr-0" id="user-menu-button" aria-expanded="false" data-dropdown-toggle="user-dropdown" data-dropdown-placement="bottom" onClick={toggleDropdown}>
                  ...
                  <img
                    ...
                    src="https://img.icons8.com/material-rounded/96/000000/user.png"
                    alt="user_photo"
                    draggable="false"
                  />
                  {user &&
              ...
              {(isOpen && user) && (
                <div className="origin-top-right absolute right-0 mt-2 w-48 rounded-md border-b border-gray-500 py-1 ring-1backdrop-blur-lg" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabIndex="-1">
                  ...
                  {user.admin && (
                    <Link to="/dashboard">
                      <span className="block px-4 py-2 text-sm " role="menuitem">Dashboard</span>
                    </Link>
                  )}
                  <Link to="/settings">
                  ...
                  <span className="block px-4 py-2 text-sm hover:cursor-pointer" role="menuitem" onClick={handleLogout}>Sign out</span>
                </div>
              )}
            </div>
          ...
          <div className={`items-center justify-between w-full lg:flex  lg:w-auto lg:order-1 ${isMobileMenuOpen ? '' : 'hidden'}`} id="mobile-menu-2">
            <ul className="flex flex-col font-medium p-4 lg:p-0 mt-4 border border-gray-100 rounded-lg lg:flex-row lg:space-x-8 lg:mt-0 lg:border-0 md:">
              ...
              <li>
                <Link to="/about-us" className="block py-2 pl-3 pr-4 rounded " onClick={closeMobileMenu}>About Us</Link>
              </li>
              <li>
                <Link to="/add" className="block py-2 pl-3 pr-4 rounded " onClick={closeMobileMenu}>View Listings</Link>
              </li>
              <li>
                <Link to="/contact-us" className="block py-2 pl-3 pr-4 rounded " onClick={closeMobileMenu}>Contact Us
                </Link>
              </li>
            </ul>
          </div>
